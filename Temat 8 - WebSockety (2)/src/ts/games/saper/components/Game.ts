...
import { IConfig, IButtons } from '../game.interfaces';
...
export class Game extends UI {
	...
	numberOfCols: number | null = null;
	numberOfMines: number | null = null;
	...
	handleElements(): void {
		this.board = <HTMLDivElement>this.getElement(this.UISelectors.board);

		this.buttons.modal = <HTMLButtonElement>(
			this.getElement(this.UISelectors.modalButton)
		);

		this.buttons.easy = <HTMLButtonElement>(
			this.getElement(this.UISelectors.easyButton)
		);

		this.buttons.normal = <HTMLButtonElement>(
			this.getElement(this.UISelectors.normalButton)
		);
	}
	...
	renderBoard(): void {
		...
		this.cells.flat().forEach((cell: Cell) => {
			...
			cell.element = <HTMLDivElement>cell.getElement(cell.selector);
		});
	}
	...
	setCellValue(cell: Cell): void {
		...
		for (
			let rowIndex: number = startValueForRow;
			rowIndex <= endValueForRow;
			rowIndex++
		) {
			for (
				let colIndex: number = startValueForCol;
	...
	setStyles(): void {
		const cellsInRow: string = this.numberOfCols.toString();
